...
export default function Home() {
  ...
  const startCamera = async () => {
    try {
      ...
      alert('Ошибка: Не удалось получить доступ к камере. Проверьте разрешения приложения.');
    }
  };
  ...
  const processPhoto = async () => {
    ...
    try {
      ...
      img.onload = async () => {
        ...
        const { data: { text } } = await Tesseract.recognize(croppedImage, 'rus', {
    ...
    <main className="mobile-container">
      ...
      <div className="mobile-controls">
        {!capturedPhoto ? (
          ...
          <>
            <button onClick={retakePhoto} className="mobile-button btn-secondary">
              Повторить
            </button>
            <button
            ...
            <div className="results-content">
              {ocrText && (
                <div className="result-section">
                  <h3 className="result-title">Текст, найденный на изображении:</h3>
